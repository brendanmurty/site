...
export async function LastFmLovedTracks(): Promise<{ artist: string; track: string; url: string; }[]> {
  ...
  const apiRequestUrl: string = lastFmApiUrl + '?method=user.getlovedtracks&user=' + lastFmUsername + '&limit=5&api_key=' + lastFmApiKey + '&format=json';
